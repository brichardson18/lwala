...
alterState(state => {
  if (
    ...
    //Deliveries
  console.log(dataValue('$.form.TT5.Child_Information.Delivery_Information')(state));
...
alterState(state => {
  ...
  const { ANCs } = TT5 && TT5.Child_Information ? TT5.Child_Information : '';
  if (ANCs && ANCs.ANC_1) {
    return upsert(
      'Service__c',
      'Service_UID__c',
...
alterState(state => {
  const { TT5 } = state.data.form;
  const { Immunizations } =
    TT5 && TT5.Child_Information ? TT5.Child_Information : '';
